"""OpenTelemetry span helpers for Bedrock Converse."""
...
TRACER_NAME = "@launchdarkly/ai-bedrock-messages"
...
def usage_of(value: Any) -> tuple[dict[str, int], SpanUsage]:
    ...
    raw = {
        "input_tokens": number_or_zero(value.get("inputTokens")),
        "output_tokens": number_or_zero(value.get("outputTokens")),
        "total_tokens": number_or_zero(value.get("totalTokens")),
    }
    cache_read = number_or_zero(value.get("cacheReadInputTokens"))
    cache_write = number_or_zero(value.get("cacheWriteInputTokens"))
    if "cacheReadInputTokens" in value:
    ...
    return raw, SpanUsage(
        input=raw["input_tokens"] + cache_read + cache_write,
...
def tool_definitions(tools: list[dict[str, Any]]) -> list[ToolDefinitionInput]:
    return [
        ToolDefinitionInput(
            name=str(tool["toolSpec"].get("name", "")),
            description=tool["toolSpec"].get("description"),
            parameters=tool["toolSpec"].get("inputSchema", {}).get("json"),
        )
        for tool in tools
    ]
