...
async def test_tool_trajectory_reaches_the_judge_via_message_history(
    ...
    async def handler(
        ...
        if "Judge" in config.get("instructions", ""):
            ...
            seen["tool_trajectory"] = variables["tool_trajectory"]
    ...
    # tool_trajectory carries the same block on its own, so a rubric can ask
    # about tool use without also restating the conversation.
    assert seen["tool_trajectory"] in history
    assert "Your order shipped." not in seen["tool_trajectory"]
...
async def test_each_row_gets_only_its_own_tool_trajectory(
    ...
    async def handler(
        ...
        if "Judge" in config.get("instructions", ""):
            histories[str(user_input)] = variables["tool_trajectory"]
            return {"output": '{"score": 1, "reasoning": "ok"}'}
...
async def test_a_row_that_called_no_tools_says_so_to_the_judge(
    ...
    async def handler(
        ...
        if "Judge" in config.get("instructions", ""):
            seen["trajectory"] = variables["tool_trajectory"]
