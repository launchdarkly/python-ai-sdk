...
"""The object kind under which skill content is delivered."""
...
def verified_bytes(
    key: str, content: str, expected_hash: str, version: int
    ...
    The second pass re-encodes and re-hashes content the first pass already
    hashed. That redundancy is deliberate: it is negligible next to the write
    it guards, and carrying the first pass's bytes forward would put a "trust
    the value computed upstream" branch inside the one function whose entire
    job is not to.
    """
    try:
        encoded = content.encode("utf-8")
    except UnicodeEncodeError:
        # json.loads turns a "\ud800" escape into an unpaired surrogate, which has
        # no UTF-8 encoding. There are no bytes the server could have hashed, so
        # this is not authentic content. Never use errors="surrogatepass" here:
        # that would fabricate bytes and could satisfy the hash comparison.
